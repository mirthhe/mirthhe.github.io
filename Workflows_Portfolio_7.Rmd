...
```{r descriptive stats}
# Normalize the amount of cases for the size of the population
...
gf_norm_continent <- gf_continent %>% mutate(cases_per_100000 = 
                                               cases/population*100000)
view(gf_norm_continent)

# Get the mean and stdev values per country/continent
...
ggplot(gd_norm_country, aes(x=year, y=cases_per_100000, 
  ...
  labs(title = "Dengue cases",
       ...
       x = "Year",
       caption = str_wrap("Figure 4.2. Scatterplot to show the average amount of 
       weekly dengue cases per 100.000 inhabitants per year per country.", 80)) +
...
ggplot(gf_summ, aes(x=reorder(continent, -avg_cases_per_100000), 
  ...
  labs(title = "Flu cases per continent",
       ...
       x = NULL,
       caption = str_wrap("Figure 4.3. The average amount of weekly flu cases per 100.000 inhabitants per continent from approx. 2003 to 2015.", 70)) +
    theme_minimal()
```
