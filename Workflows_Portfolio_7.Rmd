...
So far the subjects have been about R, BASH, HTML and CSS. Another important language to understand is SQL. With this assignment the focus is on learning the basics of SQL and applying it to relational data. To start, the dataframes will be created, using the files provided in the assignment, and tidied up.
...
```{r export}
# Export the dataframes as csv and rds
save_csv_rds(flu_df_tidy, "data/data070/flu")
save_csv_rds(dengue_df_tidy, "data/data070/dengue")
save_csv_rds(gapminder, "data/data070/gapminder")
```

----

## Connecting to DBeaver {#dbeaver}
...
![Figure 4.1. The hierarchy of the workflowsdb database including the three tables.](images/sql_tree.png)
...
```{r inspection}
# Checking the columnnames and columntypes
str(gapminder_df)
str(flu_df_tidy)
str(dengue_df_tidy)

# Checking the timeframes
...
# Checking how many countries took part
...
gd_country <- read_csv("data/data070/gapminder_dengue_country.csv")
gf_continent <- read_csv("data/data070/gapminder_flu_continent.csv")
```
...
```{r descriptive stats}
# Normalize the cases for population ----
...
# Mean and stdev per country/continent ----
...
ggplot(gd_norm_country, aes(x=year, y=cases_per_100000, 
  ...
  labs(title = "Dengue cases",
       ...
       x = "Year",
       caption =graph_cap("Figure 4.2. Scatterplot to show the average amount of 
       weekly dengue cases per 100.000 inhabitants per year per country.")) +
  scale_x_continuous(breaks = round(seq(min(gapminder_flu_dengue$year), 
                                        max(gapminder_flu_dengue$year), 
                                        by = 1),1)) +
  theme_minimal()
```
...
ggplot(gf_summ, aes(x=reorder(continent, -avg_cases_per_100000), 
  ...
  labs(title = "Flu cases per continent",
       ...
       x = NULL,
       caption = graph_cap("Figure 4.3. The average amount of weekly flu cases per 100.000 inhabitants per continent from approx. 2003 to 2015.")) +
    theme_minimal()
```
...
ggplot(gf_norm_continent, aes(x=year, y=cases_per_100000, 
  ...
  labs(title = "Weekly flu cases across the world",
       y = "Weekly flu cases",
       x = "Year",
       caption = "Figure 4.4. Scatterplot to show the average amount of 
       weekly flu cases per year per continent.") +
